...
public class Algb {
    ...
    public static double[] sub(double[] a, double[] b) {// subtracts 2 vectors
    ...
    public static double dot(double[] a, double[] b) {// dot product
        // a.b
        double resp = 0;
        for (int i = 0; i < a.length; i++) {
            resp += a[i] * b[i];
    ...
    public static double[][] matrixMatrixProduct(double[][] a, double[][] b) {
        if (a[0].length != b.length) {
            throw new RuntimeException("Inconsistent dimensions. Unable to multiply matrices");
        }
    ...
    // receives the basis change matrix (BCM) and the points matrix (PM)
    // performs: [v]a = [M]a ^b * [v]b Multiplies the BCM by the (PM)
    public static double[][] changeOfCoordinates(double[][] pontos,
                                                 double[][] MM, double[] C) {
    ...
    public static double[] barCoef(double[] p, double[] v0, double[] v1, double[] v2) {
        ...
        double area2 = getMagnitude(n);
        double[] e1 = sub(v2, v1);
        double[] vp1 = sub(p, v1);
        double[] c = cross(e1, vp1);

        u = getMagnitude(c) / area2;
        if (dot(n, c) < 0) {
            u = -u;
        }
        double[] e2 = sub(v0, v2);
    ...
    public static double[] refract(double[] incident, double[] normal, double n1, double n2) {
        ...
        double s1 = getMagnitude(k);
        double s2 = s1 * n1 / n2;
        if (s2 <= 1) {
            double c2 = Math.sqrt(1 - s2 * s2);
            refract = new double[4];
            refract = add(refract, dotByScale(s2 / s1, k));
            refract = add(refract, dotByScale(c2, normalize(proj)));
        } else {
